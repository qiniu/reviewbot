...
func TestIsGopAutoGeneratedFile(t *testing.T) {
	tcs := []struct {
		input    string
		expected bool
	}{
		{"cdn-admin.v2/client/dns/gop_autogen.go", true},
		{".", false},
		{"gop_autogen_test.go", true},
		{"gop_autogen2_test.go", true},
		{"c2go_autogen.go", true},
		{"/c2go_autogen.go", true},
		{"/abc.go", false},
	}

	for _, tc := range tcs {
		actual := isGopAutoGeneratedFile(tc.input)
		if actual != tc.expected {
			t.Errorf("expected: %v, got: %v", tc.expected, actual)
		}
	}
