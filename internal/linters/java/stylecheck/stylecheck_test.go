...
func TestForConfig(t *testing.T) {
	...
	path, err := styleRuleCheck(xlog.New("ut"), "https://raw.githubusercontent.com/checkstyle/checkstyle/master/src/main/resources/sun_checks.xml")
	if err != nil {
...
func TestStyleRuleCheck(t *testing.T) {
	...
	tc := []struct {
		...
		{
			input:    "",
			expected: dir + "/config/linters-config/.java-sun-checks.xml",
			err:      nil,
		},
		{
	...
	for _, c := range tc {
		got, err := styleRuleCheck(xlog.New("ut"), c.input)
...
func TestCheckJar(t *testing.T) {
	var stykejarfilename = "checkstyle.jar"
	filePath, _ := os.Getwd()
