...
func TestForConfig(t *testing.T) {
	fileDir, _ := os.Getwd()
	rulefiledirpath := filepath.Join(fileDir, "tmp")
...
func TestStyleRuleCheck(t *testing.T) {
	...
	tc := []struct {
		...
		{
			input:    "",
			expected: dir + "/tmp/sun_checks.xml",
			err:      nil,
		},
		{
...
func TestCheckJar(t *testing.T) {
	...
	filePath, _ := os.Getwd()
	filename2 := filepath.Join(filePath, "tmp", stykejarfilename)
