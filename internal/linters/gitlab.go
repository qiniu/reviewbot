...
func ListMergeRequestsFiles(ctx context.Context, gc *gitlabapi.Client, owner string, repo string, number int, pid int) ([]*gitlabapi.MergeRequestDiff, *gitlabapi.Response, error) {
	...
	var pullRequestAffectedFiles []*gitlabapi.MergeRequestDiff

	for {
		files, response, err := gc.MergeRequests.ListMergeRequestDiffs(pid, number, opts)
		if err != nil {
			return nil, nil, err
		}
...
func FilterMergeRequestWithCommit(ctx context.Context, gc *gitlabapi.Client, pid, owner, repo, headSha string) ([]*gitlabapi.MergeRequest, error) {
	var allPRs []*gitlabapi.MergeRequest
	var state = "opened"
	...
	for {
		...
		if resp.StatusCode != http.StatusOK {
			return nil, fmt.Errorf("failed to list pull requests: %v", gitlabapi.Stringify(resp.Body))
		}
...
func CreateMergeReviewComments(ctx context.Context, gc *gitlabapi.Client, owner string, repo string, number int, comments []*gitlabapi.MergeRequest) ([]*github.PullRequestComment, error) {
...
func DeleteMergeReviewComments(ctx context.Context, gc *github.Client, owner, repo string, comments []*github.PullRequestComment) error {
	for _, comment := range comments {
		cmt := comment
		err := RetryWithBackoff(ctx, func() error {
...
func constructMergeRequestComments(linterOutputs map[string][]LinterOutput, linterName, commitID string) []*github.PullRequestComment {
