...
func (s *GitlabServer) gitlabRequestHandle(w http.ResponseWriter, r *http.Request) {
...
func (s *GitlabServer) processPullRequestEvent(ctx context.Context, event *github.PullRequestEvent) error {
	log := lintersutil.FromContext(ctx)
...
func (s *GitlabServer) processCheckRunRequestEvent(ctx context.Context, event *github.CheckRunEvent) error {
	log := lintersutil.FromContext(ctx)
...
func (s *GitlabServer) processCheckSuiteEvent(ctx context.Context, event *github.CheckSuiteEvent) error {
	log := lintersutil.FromContext(ctx)
	if event.GetAction() != "rerequested" {
		log.Debugf("skipping action %s\n", event.GetAction())
		return nil
...
func (s *GitlabServer) handle(ctx context.Context, event *github.PullRequestEvent) error {
	var (
...
func (s *GitlabServer) GithubClient(installationID int64) *github.Client {
	if s.accessToken != "" {
