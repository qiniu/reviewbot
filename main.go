...
func main() {
	...
	if o.linterReferencePath == "" {
		abspath, exist := lintersutil.FileExists(filepath.Join("config", "linters-config", "linter-reference.json"))
		if !exist {
			log.Fatalf("linter reference file not found: %s", o.linterReferencePath)
		}
		o.linterReferencePath = abspath
	}
	...
	s := &Server{
		...
		linterReference:  linterReference,
	}
